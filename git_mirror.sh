...
pat2b64() {
    # -w0: no line wrapping. base64 wraps at 76 cols by default, so a PAT long
    # enough to exceed it produces a multi-line value that ends up as a newline
    # inside git http.extraHeader -> "curl was given a bad argument". Longer,
    # API-generated PATs hit this; short classic PATs happened to fit in one line.
    pat_b64=$(echo -n ":$1" | base64 -w0)
    echo "$pat_b64"
}
